...
# توجه: این داده‌ها با ریستارت شدن ربات (مثلا روی Railway) از بین می‌روند.
user_warnings = {}
muted_users = set()
...
# هندلر دکمه‌های شیشه‌ای (Inline Keyboard) — قبلاً ثبت نشده بود
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "ai_mode":
        await query.message.reply_text(
            "🤖 برای صحبت با هوش مصنوعی، دستور زیر را بفرست:\n`/ai متن سوال شما`",
            parse_mode="Markdown",
        )
...
async def ai_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_prompt:
        await update.message.reply_text(
            "❌ لطفاً پیام خود را بعد از دستور وارد کنید. مثال:\n`/ai چطور پایتون یاد بگیرم؟`",
            parse_mode="Markdown",
        )
        return
    ...
    try:
        ...
        await context.bot.edit_message_text(
            ...
            text=f"❌ خطایی در دریافت پاسخ از هوش مصنوعی رخ داد.\n`{e}`",
            parse_mode="Markdown",
        )
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # نام فایل یکتا برای هر کاربر/درخواست تا تصاویر همزمان با هم تداخل نکنند
    ...
    try:
        ...
        response = client.models.generate_content(model=MODEL_NAME, contents=[caption, img])
        await context.bot.edit_message_text(
            chat_id=update.message.chat_id, message_id=sent_msg.message_id, text=response.text
        )
    except Exception as e:
        await context.bot.edit_message_text(
            chat_id=update.message.chat_id,
            message_id=sent_msg.message_id,
            text=f"❌ خطایی در پردازش تصویر پیش آمد.\n`{e}`",
            parse_mode="Markdown",
        )
    finally:
